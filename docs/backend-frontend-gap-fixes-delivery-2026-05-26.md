...
- 以下内容记录的是当时的修复思路。
- 当前产品约束已经调整为：runtime 日志与原始运行元数据可能包含敏感信息，不上传 MinIO，不登记到对外 artifacts，不提供前端下载。
- 后续 agent 应以“保留状态事件与错误摘要、仅归档可交付产物”为准。

修复内容：

- 新增结构化事件记录，用于保留任务阶段变化、失败信息、取消信息。
- 失败场景下至少应保留：
  - 数据库中的 `task_events`
  - `translation_tasks.error_message`
  - 已成功生成并允许对外交付的 artifacts
- `task-config.json`、`task.log`、`task-events.jsonl` 仅保留在本地 runtime 目录，不上传 artifact。

实现位置：

- `backend/app/workers/translation_runner.py`

对应对外交付 artifact 类型应以业务产物为主，而不是 runtime 调试文件。
