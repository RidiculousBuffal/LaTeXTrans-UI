...
- **Success Criteria**:
  ...
  - 任务产生的输入源文件、翻译产物、日志和元数据必须完成归档，归档完整率达到 `100%`。
...
- **项目维护者/管理员**: 需要定位失败任务、查看日志、管理归档文件、验证服务运行状态。
...
- **Story 3**: 作为团队用户，我希望系统自动归档源文件、翻译结果、日志和元数据，从而便于后续追溯和复用。
...
- **For Story 3**
  ...
  - 每个任务必须将源文件、翻译后的 LaTeX 工程、最终 PDF、关键日志上传到 MinIO。
  - 任务详情页必须提供归档产物下载入口。
  - 系统必须记录文件对象路径、版本信息和上传时间。

- **For Story 4**
  - 历史任务列表支持按 `arXiv ID`、任务状态、提交人、时间范围检索。
  - 用户可以进入任务详情页查看任务参数、过程日志和最终产物。
  - 系统对同一 `arXiv ID` 的历史任务提供聚合展示。

- **For Story 5**
  - 失败任务必须展示失败阶段、异常摘要和日志下载入口。
...
- **MinIO**
  - 保存原始输入、解压后的源文件、翻译后的工程目录、PDF、日志和中间 JSON 产物。
...
6. 任务成功后，将 PDF、翻译工程、日志和中间产物上传到 MinIO，写入 MySQL 元数据，状态置为 `SUCCEEDED`。
7. 若任务失败，写入错误摘要和失败日志，状态置为 `FAILED`。
...
- **MinIO**
  - 用于保存源文件、输出 PDF、翻译工程、日志、JSON 中间文件。
